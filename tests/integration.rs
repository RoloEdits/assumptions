...
#[test]
fn assumption_location_accessor_points_to_call_site() {
    let err = Assumption::new("episode should have at least one panel".to_string());
    assert_eq!(err.location().file(), "tests/integration.rs");
    assert_eq!(err.location().line(), 515);
}

#[test]
fn assumption_location_accessor_points_to_call_site_using_assumption_macro() {
    let result: Result<(), Assumption> = (|| {
        assumption!("episode should have at least one panel");
    })();
    let err = result.unwrap_err();
    assert_eq!(err.location().file(), "tests/integration.rs");
    assert_eq!(err.location().line(), 523);
}

#[test]
fn assumption_location_accessor_points_to_call_site_using_assume_macro() {
    ...
    let err = result.unwrap_err();
    assert_eq!(err.location().file(), "tests/integration.rs");
    assert_eq!(err.location().line(), 534);
}
#[test]
fn assumption_location_accessor_points_to_call_site_using_assume_matches_macro() {
    ...
    let err = result.unwrap_err();
    assert_eq!(err.location().file(), "tests/integration.rs");
    assert_eq!(err.location().line(), 545);
}
#[test]
fn assumption_location_accessor_points_to_call_site_using_assume_eq_macro() {
    let cond = std::hint::black_box(false);
    let result: Result<(), Assumption> = (|| {
    ...
    let err = result.unwrap_err();
    assert_eq!(err.location().file(), "tests/integration.rs");
    assert_eq!(err.location().line(), 556);
}
#[test]
fn assumption_location_accessor_points_to_call_site_using_assume_ne_macro() {
    let cond = std::hint::black_box(false);
    let result: Result<(), Assumption> = (|| {
    ...
    let err = result.unwrap_err();
    assert_eq!(err.location().file(), "tests/integration.rs");
    assert_eq!(err.location().line(), 567);
}
